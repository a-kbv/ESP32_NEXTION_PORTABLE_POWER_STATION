...
int calculateBatteryPercentage(float threeCellVoltage) {
...
unsigned long currentMillis = millis();
  if (currentMillis - previousMillis >= interval)
  {
    ...
    int batteryPercentage = calculateBatteryPercentage(outputVoltage);
...
//EXAMPLE READ NEXTION DATA
  String dataFromDisplay = "";
   if(Serial2.available())
   {
    ...
    {
      dataFromDisplay += char(Serial2.read());
    }
    Serial.println(dfd);
      if( dataFromDisplay == "on")
       { digitalWrite(2,HIGH);}
      else if( dataFromDisplay == "off")
       { digitalWrite(2,LOW); }
   }
//EXAMPLE READ NEXTION DATA
